...
export default function UserNewForm({ isEdit, currentUser,listUser,id }) {
  const navigate = useNavigate();
    var [checkBoxStyle, setCheckBoxStyle] = React.useState([]);
   const [valueHeight, setValueHeight] = React.useState(isEdit ? [] : [0,0]);
   const [valueAge, setValueAge] = React.useState(isEdit ? [] : [0,0]);
   const [valueMeasure1, setValueMeasure1] = React.useState(isEdit ? [] : [0,0]);
   const [valueMeasure2, setValueMeasure2] = React.useState(isEdit ? [] : [0,0]);
   const [valueMeasure3, setValueMeasure3] = React.useState(isEdit ? [] : [0,0]);
...
var [checkBoxSkin, setCheckBoxSkin] = React.useState([
  ...
  console.log(valueMeasure3, " valueMeasure3");
 ...
 if(!isCheckSex){
   ...
   React.useEffect(() => {
    ...
    if(isEdit){
       if(currentUser !== undefined){
        ...
        cookies.set('USER_ID', currentUser.id);
      }
       fetch(`https://api.pimo.studio/api/v1/castings/information/${CookiesJS.get('USER_ID')}`)
    .then(res=>res.json())
    .then(json=>{
      console.log('acbbb ',json)
       if(checkBoxHair.length > 0 && json.listRequest !== undefined) {
        for(var m = 0; m < checkBoxHair.length; m++){
          for(var n = 0; n < json.listRequest.hairList.length; n++){
              if(checkBoxHair[m].name.trim() ===  json.listRequest.hairList[n].trim()){
                checkBoxHair[m].checked = true;
              }
          }
          setCheckBoxHair(checkBoxHair)
         }
        
      }
      if(checkBoxSkin.length > 0 && json.listRequest !== undefined) {
        for(var j = 0; j < checkBoxSkin.length; j++){
          for(var k = 0; k < json.listRequest.skinList.length; k++){
              if(checkBoxSkin[j].name.trim() ===  json.listRequest.skinList[k].trim()){
                checkBoxSkin[j].checked = true;
              }
          }
          setCheckBoxSkin(checkBoxSkin)
         }
       
      }
      if(json.listRequest !== undefined){
        setValueHeight([json.listRequest.heightFrom,json.listRequest.heightTo])
        setValueAge([json.listRequest.ageFrom,json.listRequest.ageTo])
        setValueMeasure1([json.listRequest.bustFrom,json.listRequest.bustTo])
         setValueMeasure2([json.listRequest.hipFrom,json.listRequest.hipTo])
         setValueMeasure3([json.listRequest.waistFrom,json.listRequest.waistTo])  
      }else{
        setValueHeight([0,0])
        setValueAge([0,0])
        setValueMeasure1([0,0])
        setValueMeasure2([0,0])
        setValueMeasure3([0,0])  
      }
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting, resetForm, setErrors }) => {
      ...
      checkBoxStyle.map((value, index) => {
        ...
        if(isEdit){
          console.log('update ')
          var request='';
          request += "<br/>Chiều cao: "+ valueHeight[0] + "-" + valueHeight[1]+" (cm)<br/>"
          ...
          + "Màu Tóc: "+  HAIR_NAME + "<br/>"
          + "Màu Da: "+  SKIN_NAME
        var statusCasting = false;
      if(new Date().getDate() < new Date(values.city).getDate() && values.status === 'active'){
          statusCasting = true;
      }else if(new Date().getDate() > new Date(values.city).getDate() && values.status === 'banned'){
          statusCasting = false;
      }else if(new Date().getDate() < new Date(values.city).getDate() && values.status === 'banned'){
        statusCasting = false;
    }
      ...
      formData.append('status', statusCasting)
      ...
      formData.append('genders', SEX)
       ...
       formData.append('linkPoster', values.avatarUrl)
    ...
    try {
      const { data } = await axios.put('https://api.pimo.studio/api/v1/castings', formData, axiosConfig);
       if(data.success) {
        result = true;
     }else{
       result = false;
         ...
         result = false;
          ...
          console.log('create ')
          values.country += "<br/>Chiều cao: "+ valueHeight[0] + "-" + valueHeight[1]+" (cm)<br/>"
          + "Tuổi: "+ valueAge[0] + "-" + valueAge[1] + "<br/>"
          + "Vòng 1: "+ valueMeasure1[0] + "-" + valueMeasure1[1]+" (cm)<br/>"
          + "Vòng 2: "+ valueMeasure2[0] + "-" + valueMeasure2[1]+" (cm)<br/>"
          + "Vòng 3: "+ valueMeasure3[0] + "-" + valueMeasure3[1]+" (cm)<br/>"
            ...
            <Card sx={{ p: 3 }}>
              <Stack spacing={3}>
               ...
               <TextField
                ...
                >
                  <p>Mức lương</p>
                  <p style={{ marginLeft: "21rem" }}> Chiều cao</p>
                 </Stack>
                ...
                >
                  ...
                  />
              <SliderForm value={valueHeight} setValue={setValueHeight}/>
